...
const TYPE = {
  id_proof:            { label: 'ID Proof',            icon: FileBadge,     tile: 'bg-blue-50 text-blue-600',       badge: 'blue' },
  agreement:           { label: 'Agreement',           icon: FileSignature, tile: 'bg-brand-50 text-brand-600',     badge: 'indigo' },
  payment_proof:       { label: 'Payment Proof',       icon: Receipt,       tile: 'bg-amber-50 text-amber-600',     badge: 'yellow' },
  police_verification: { label: 'Police Verification', icon: ShieldCheck,   tile: 'bg-emerald-50 text-emerald-600', badge: 'green' },
  other:               { label: 'Other',               icon: FileText,      tile: 'bg-slate-100 text-slate-500',    badge: 'gray' },
};
const typeStyle = (t) => TYPE[t] || { label: t, icon: FileText, tile: 'bg-slate-100 text-slate-500', badge: 'gray' };
                ...
                <div className="group h-full bg-white rounded-2xl border border-slate-200/70 shadow-card hover:shadow-soft hover:-translate-y-0.5 transition-all duration-200 p-5 flex flex-col">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-slate-900 truncate" title={d.fileName}>{d.fileName}</p>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-4 inline-flex items-center justify-center gap-2 h-9 rounded-lg border border-slate-300 text-sm font-semibold text-slate-700 hover:bg-brand-50 hover:border-brand-300 hover:text-brand-700 transition-colors"
                  >
