...
function formatCell(key, value) {
  ...
  if (MONEY_KEYS.has(lk)) return <span className="font-semibold text-slate-800 dark:text-slate-200 tabular-nums">{inr(toNum(value))}</span>;
            ...
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-full transition-all ${
                active === r.slug
                  ...
                  : 'text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/10'
              }`}
            >
              {r.label}
            </button>
